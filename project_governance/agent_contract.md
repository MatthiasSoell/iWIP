...
- `REVEAL FINAL` finalisiert Reveal mit allen Mindestpruefungen und fuehrt nach der inhaltlichen Finalisierung das technische Content-Emoji-Postprocessing ueber `prompts/content_emojis_reveal.md` auf der aktuellen `_index.md` aus. Wurde der Agent mit `/PLAN FORSCHUNG` gestartet, folgt der Abschluss der Forschungsartefakte ausschliesslich dem Abschnitt `Optionaler Zusatzmodus: Forschung` und bleibt dabei vom Gate `BLOG FINAL` getrennt.
