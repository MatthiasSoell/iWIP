...
- `/PLAN FORSCHUNG` startet denselben Planungsdialog, aktiviert zusaetzlich aber als optionalen Zusatzmodus die interne Forschungsprotokollierung; im ersten Turn werden dabei keine Dateien, Snapshots oder Builds erzeugt. Persistente Forschungsartefakte in `exports/research/` werden erst ab `BLOG GO` geschrieben; weitere Forschungsbefehle werden dadurch nicht eingefuehrt.
...
| `/PLAN FORSCHUNG` | Expliziter Planungsbefehl mit Forschungsmodus | Forschungsmodus ist gewuenscht; Standardgates bleiben unveraendert | Wie `/PLAN` planen und zusaetzlich interne Forschungsprotokollierung aktivieren (zunaechst ohne Dateischreibzugriff) | Vor `BLOG GO` Dateien in `exports/research/` erzeugen, sichtbare Zielartefakte erzeugen, den Forschungsmodus als Ersatz fuer `BLOG GO` oder `REVEAL GO` behandeln, zusaetzliche Forschungsbefehle einfuehren | Forschungsmodus aktivieren |
...
Wenn ein Fall mit `/PLAN FORSCHUNG` beginnt, startet die Forschungsdokumentation intern ab diesem Zeitpunkt. Persistente Forschungsdateien in `exports/research/<case_id>/` werden erst ab `BLOG GO` fortlaufend geschrieben. Vorgesehene Dateien sind `chat_log.md`, `planning_trace.md`, `decision_log.md`, `metadata.yaml`, `blog_snapshot.txt`, `reveal_snapshot.txt`, `index.md` und `_index.md`; tatsaechlich erzeugte Dateien haengen vom Fallverlauf ab. Ohne aktivierten Forschungsmodus duerfen spaetere Auswertungen Zeitpunkte nur rekonstruieren und muessen fehlende Marker oder Rohdaten explizit als nicht verlaesslich verfuegbar kennzeichnen.
...
Vor dem finalen Build-/Release-Check wird nach der inhaltlichen Finalisierung das technische Content-Emoji-Postprocessing ueber `prompts/content_emojis_blog.md` auf der aktuellen `index.md` ausgefuehrt.
...
Vor dem finalen Build-/Release-Check wird nach der inhaltlichen Finalisierung das technische Content-Emoji-Postprocessing ueber `prompts/content_emojis_reveal.md` auf der aktuellen `_index.md` ausgefuehrt.
